...
def delete_db_file(filename):
    try:
        os.remove(filename)
        print(
            "Old database at file %s delete_db_filed. Generating new database.",
            filename,
        )
    except OSError as e:
            ...
            with codecs.open(filename, encoding="utf-8-sig") as f:
                ...
                if author_is_ignored:
                    ...
                    for sentence in sentences:
                        insert_into_sentence_table(
                            c, article_id, author_id, data, sentence, plags
                        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generates a sqlite database from the PAN corpus training subset."
    )
    parser.add_argument(
        "--srcdir",
