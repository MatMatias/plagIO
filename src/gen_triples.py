...
def get_stvecs_for_doc(doc_id):
    stvecs_filename = "./data/tuples/tuples{:05d}.pkl".format(doc_id)
...
def main():
    ...
    parser.add_argument(
        "--pandb", help="specify the sqlite PAN database file", required=True
    )
    parser.add_argument(
        "--srcdir", help="source directory of tuples files", required=True
    )
    parser.add_argument(
        "--destdir",
    ...
    for doc_id in document_ids_set:
        ...
        triples_filename = os.path.join(
            args.destdir, "triples{:05d}.pkl".format(doc_id)
        )
        ...
        for cur_tuple in tuples:
            ...
            example = (np.asarray(vec1), np.asarray(vec2), similarity_bit)
